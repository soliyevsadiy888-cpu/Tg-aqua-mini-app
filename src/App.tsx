...
/* ============================================================
   ...
   ============================================================ */
function AppleEmoji({ e, size = 24, style }) {
  if (!e) return null;
...
function Sticker({ e, size = 40, radius, bg = "#0A1620", ring = false, borderColor, title }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: r,
        background: bg,
        border: `1px solid ${borderColor || (ring ? "#1C3A4A" : "rgba(255,255,255,0.08)")}`,
...
function IconBadge({ icon, size = 34, grad = "linear-gradient(135deg, #00C9B1, #00A896)", iconSize, color = "#fff" }) {
  return (
    <div
      style={{
        width: size,
...
function Welcome({ onNext }) {
  return (
    ...
    >
      <Bubbles count={16} />
      <div style={{ marginBottom: 8, animation: "swim 3s ease-in-out infinite" }}><AppleEmoji e="🐠" size={56} /></div>
      <h1
        style={{
          fontSize: 32,
          fontWeight: 800,
      ...
      <div style={{ display: "flex", gap: 18, marginBottom: 36 }}>
        {[
          ...
          <div key={t} style={{ width: 84 }}>
            <div style={{ display: "flex", justifyContent: "center", marginBottom: 4 }}><Sticker e={icon} size={36} /></div>
            <div style={{ fontSize: 12, fontWeight: 700, marginTop: 4, textAlign: "center" }}>{t}</div>
            <div style={{ fontSize: 11, color: "#6C8E96", textAlign: "center" }}>{s}</div>
          </div>
        ))}
      </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: gridCols, gap: 0, padding: "14px 18px 0", overflowX: "auto" }}>
          ...
          {list.map(fish => (
            <div key={fish.id} style={{ textAlign: "center", padding: "0 4px", position: "relative" }}>
              ...
              <div style={{ display: "flex", justifyContent: "center" }}><Sticker e={fish.img} size={48} /></div>
      ...
      {compareList.length > 0 && (
        <div style={{
          ...
          <div style={{ display: "flex", gap: -4 }}>
            {compareList.map((f) => (
              <span key={f.id} style={{ marginRight: -4, display: "inline-block" }}><AppleEmoji e={f.img} size={18} /></span>
            ))}
          </div>
          <div style={{ flex: 1 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        {items.map(({ icon, title, desc, accent }) => (
          <div key={title} style={{ background: "#0B1B28", border: `1px solid ${accent}22`,
            ...
            <div style={{ marginBottom: 6 }}><Sticker e={icon} size={34} /></div>
  ...
  return (
    <div style={{ position: "relative", height: 120, overflow: "hidden", marginBottom: 16 }}>
      ...
      {fishes.map((f) => (
        ...
        }}><AppleEmoji e={f.img} size={f.size} /></span>
      ))}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8 }}>
          {TIME_SLOTS.map((s) => (
            <button key={s.id} onClick={() => setSlot(s.id)}
              ...
              <div style={{ display: "flex", justifyContent: "center", marginBottom: 4 }}><Sticker e={s.icon} size={30} /></div>
    ...
    <div style={{ minHeight: "100vh", background: "#08131F", color: "#E8F4F8", paddingBottom: 30 }}>
      ...
      <div style={{ padding: "0 16px" }}>
        ...
        <div style={{ marginTop: 28, marginBottom: 8 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            {[
              { icon: "doctor", grad: "linear-gradient(135deg, #00C9B1, #00A896)", label: "AI-доктор рыб", sub: "Диагноз по симптомам", action: onOpenDoctor, accent: "#00C9B1" },
              { icon: "edit", grad: "linear-gradient(135deg, #4D9FFE, #2F6FCB)", label: "Дневник аквариума", sub: "Уход и напоминания", action: onOpenDiary, accent: "#9FC4CC" },
              { icon: "cart", grad: "linear-gradient(135deg, #F0A93C, #C97F1F)", label: "Кабинет продавца", sub: "Продавайте рыб", action: onOpenSeller, accent: "#F0A93C" },
              { icon: "truck", grad: "linear-gradient(135deg, #9FC4CC, #5D8A93)", label: "Кабинет курьера", sub: "Доставляйте заказы", action: onOpenCourier, accent: "#9FC4CC" },
              { icon: "group", grad: "linear-gradient(135deg, #8B5CF6, #6D28D9)", label: "Клуб аквариумистов", sub: "Общение и советы", action: onOpenClub, accent: "#9FC4CC" },
              { icon: "settings", grad: "linear-gradient(135deg, #F0A93C, #C97F1F)", label: "Admin-панель", sub: "Управление системой", action: onOpenAdmin, accent: "#F0A93C" },
            ].map(s => (
              <button
                key={s.label}
                onClick={s.action}
              ...
              >
                <div style={{ marginBottom: 8 }}><IconBadge icon={s.icon} grad={s.grad} size={38} /></div>
  ...
  return (
    <div style={{ background: S.card, border: `1px solid ${product.active ? S.border : "#0D1E2C"}`, borderRadius: 14, padding: "12px 14px", marginBottom: 10, opacity: product.active ? 1 : 0.6 }}>
      <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
        <Sticker e={product.emoji} size={48} radius={12} bg={`radial-gradient(circle, ${product.tone}33, #050B12)`} />
        <div style={{ flex: 1, minWidth: 0 }}>
    ...
    <div style={{ padding: "40px 20px", textAlign: "center" }}>
      ...
      <div style={{ background: S.card, border: `1px solid ${S.border}`, borderRadius: 14, padding: "16px", marginBottom: 24, textAlign: "left" }}>
        <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
          <Sticker e={photo?.emoji || "🐠"} size={56} radius={12} bg={`radial-gradient(circle, ${photo?.tone || "#F0A93C"}33, #050B12)`} />
          <div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 12 }}>
                {searched.map(p => (
                  <div key={p.id} onClick={() => setChosenPhoto(p.id)} style={{ background: "#0B1A27", border: `2px solid ${chosenPhoto === p.id ? S.teal : S.border}`, borderRadius: 12, overflow: "hidden", cursor: "pointer", position: "relative" }}>
                    <div style={{ aspectRatio: "1/1", background: `radial-gradient(circle at 50% 35%, ${p.tone}33, #050B12 75%)`, display: "flex", alignItems: "center", justifyContent: "center" }}><AppleEmoji e={p.emoji} size={40} /></div>
        ...
        <div style={{ padding: "16px 16px 0" }}>
          ...
          {photo && (
            <div style={{ background: S.card, border: `1px solid ${S.border}`, borderRadius: 14, padding: "12px", marginBottom: 16, display: "flex", gap: 12, alignItems: "center" }}>
              <Sticker e={photo.emoji} size={52} radius={12} bg={`radial-gradient(circle, ${photo.tone}33, #050B12)`} />
              <div>
  ...
  return (
    <div style={{animation:"slideUpDoc 0.3s ease"}}>
      ...
      <div onClick={()=>fileRef.current.click()} style={{
        ...
        {photoData?(
          <div><div style={{marginBottom:6,display:"flex",justifyContent:"center"}}><IconBadge icon="camera" size={40} grad="linear-gradient(135deg, #00C9B1, #00A896)" /></div>
          ...
          <div><div style={{marginBottom:6,display:"flex",justifyContent:"center"}}><IconBadge icon="camera" size={40} grad="linear-gradient(135deg, #4D9FFE, #2F6FCB)" /></div>
      ...
      {DIARY_BADGES.map(b => {
        ...
        return (
          <div key={b.id} title={b.title} style={{ position: "relative", flex: "0 0 auto", opacity: earned ? 1 : 0.35, filter: earned ? "none" : "grayscale(1)" }}>
            <Sticker e={b.icon} size={50} radius={14} bg={earned ? "#0F2A26" : "#102433"} borderColor={tierColor} />
            {earned && (
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, position: "relative" }}>
        <Sticker e={badge.icon} size={42} radius={12} bg="#102433" borderColor={nextTierColor} />
        <div style={{ flex: 1, minWidth: 0 }}>
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(5,10,16,0.75)", zIndex: 300, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: "#0B1B28", borderRadius: "20px 20px 0 0", padding: "24px 20px 32px", width: "100%", maxWidth: 420, color: Dp.text }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
          {EMOJIS.map(e => (
            <button key={e} onClick={() => setEmoji(e)} style={{ width: 38, height: 38, borderRadius: 10, background: emoji === e ? Dp.teal + "33" : "#102433", border: `1px solid ${emoji === e ? Dp.teal : Dp.border}`, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", padding: 0 }}><AppleEmoji e={e} size={20} /></button>
          ))}
        </div>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
            {[
              ...
              <div key={i} style={{ flex: 1, background: Dp.card, border: `1px solid ${Dp.border}`, borderRadius: 12, padding: "10px 8px", textAlign: "center" }}>
                <div style={{ display: "flex", justifyContent: "center", marginBottom: 4 }}><Sticker e={stat.icon} size={30} /></div>
      ...
      {badgeToast && (
        <>
          ...
          <div style={{ position: "fixed", top: 20, left: "50%", transform: "translateX(-50%)", width: "calc(100% - 32px)", maxWidth: 380, background: "#0F2A26", border: `1px solid ${TIER_COLORS[badgeToast.tierIndex] || Dp.teal}`, borderRadius: 14, padding: "12px 16px", zIndex: 660, boxShadow: `0 8px 28px rgba(0,0,0,0.55), 0 0 0 3px ${TIER_COLORS[badgeToast.tierIndex] || Dp.teal}33`, animation: "aquaToastDrop 0.35s cubic-bezier(0.34,1.56,0.64,1)", boxSizing: "border-box" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <span style={{ display: "inline-block", animation: "aquaBadgePulse 0.9s ease-in-out 0.1s" }}><Sticker e={badgeToast.icon} size={40} bg={(TIER_COLORS[badgeToast.tierIndex] || Dp.teal) + "22"} /></span>
              <div style={{ flex: 1, minWidth: 0 }}>
  ...
  return (
    <div style={{ background: Dp.card, border: `1px solid ${Dp.border}`, borderRadius: 16, padding: 16, marginBottom: 12 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
        <div style={{ width: 32, height: 32, borderRadius: 999, background: "#102433", border: `1px solid ${Dp.border}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      {rows.map((u, i) => (
        <div key={u.id} style={{ display: "flex", alignItems: "center", gap: 10, background: u.isMe ? "#0F2A26" : Dp.card, border: `1px solid ${u.isMe ? Dp.teal : Dp.border}`, borderRadius: 14, padding: "10px 12px", marginBottom: 8 }}>
          <div style={{ width: 26, display: "flex", alignItems: "center", justifyContent: "center" }}>
            {i < 3 ? <AppleEmoji e={medals[i]} size={18} /> : <span style={{ fontSize: 13, fontWeight: 800, color: Dp.muted }}>{`#${i + 1}`}</span>}
          </div>
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: "#0B1B28", borderRadius: "20px 20px 0 0", padding: "20px 16px 28px", width: "100%", maxWidth: 420, color: Dp.text }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 18 }}>
          {POST_PHOTO_EMOJIS.map(e => (
            <button key={e} onClick={() => setPhotoEmoji(photoEmoji === e ? null : e)} style={{ width: 40, height: 40, borderRadius: 10, background: photoEmoji === e ? Dp.teal + "33" : "#102433", border: `1px solid ${photoEmoji === e ? Dp.teal : Dp.border}`, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", padding: 0 }}><AppleEmoji e={e} size={20} /></button>
          ))}
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={onClose} style={{ flex: 1, background: "#102433", border: `1px solid ${Dp.border}`, color: Dp.soft, borderRadius: 12, padding: 12, fontSize: 14, cursor: "pointer" }}>Отмена</button>
    ...
    <div style={{ minHeight: "100vh", background: "#08131F", color: "#E8F4F8", fontFamily: "system-ui, -apple-system, sans-serif", paddingBottom: 90 }}>
      ...
      <div style={{ padding: "16px 16px 0" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          >
            <span style={{ color: "#6C8E96", display: "flex" }}><Icon name="search" size={16} /></span>
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", padding: "32px 24px" }}>
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          <div style={{ marginBottom: 8, display: "flex", justifyContent: "center" }}><AppleEmoji e={roleIcon} size={52} /></div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
            {[
              ...
              <div key={i} style={{ background: A.card, border: `1px solid ${A.border}`, borderRadius: 14, padding: "14px 12px" }}>
                <div style={{ marginBottom: 4 }}><Sticker e={s.icon} size={32} /></div>
